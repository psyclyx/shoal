...
const prepared_manifest_seed: u64 = 0x53484f414c505245;
const sparkline_fade_slices: usize = 4;
...
pub const Renderer = struct {
    ...
    pub fn deinit(self: *Renderer) void {
        ...
        if (self.sparkline_picture) |picture| {
            picture.deinit();
    ...
    pub fn drawSparkline(
        ...
        const op = SparklineOp{
            ...
            .scroll = std.math.clamp(scroll, 0.0, 1.0),
            .mirror = mirror,
        };
        self.addSparklineInstances(op) catch |err| {
            log.warn("draw sparkline failed: {}", .{err});
        };
    }
    ...
    pub fn flush(self: *Renderer) void {
        ...
        self.changed_keys.resize(self.allocator, resource_entry_count) catch |err| {
            log.warn("resource diff allocation failed: {}", .{err});
            self.clearBatchResources();
            return;
        };

        const manifest_key = resourceManifestKey(&resources);
        const current_prepared: ?*const snail.PreparedResources = if (self.prepared) |*current| current else null;
        const plan = self.gl.planResourceUpload(
            current_prepared,
            &resources,
            self.changed_keys.items[0..resource_entry_count],
        ) catch |err| {
            log.warn("resource upload planning failed: {}", .{err});
            self.clearBatchResources();
            return;
        };

        const manifest_matches = if (self.prepared_key) |key| key == manifest_key else false;
        const resource_changed_count = plan.changedKeys().len;
        const upload_allocated_bytes = plan.upload_footprint.allocatedBytes();
        const upload_used_bytes = plan.upload_footprint.usedBytes();
        var resource_uploaded = false;
        var upload_ms: f64 = 0;
        var prepared: *snail.PreparedResources = undefined;
        if (manifest_matches and resource_changed_count == 0 and self.prepared != null) {
            prepared = if (self.prepared) |*current| current else unreachable;
        } else {
            const upload_start_ns = trace.nowNs();
            var pending = self.gl.beginResourceUpload(self.allocator, plan) catch |err| {
                log.warn("resource upload begin failed: {}", .{err});
                self.clearBatchResources();
                return;
            };
            defer pending.deinit();
            pending.record(.{}, .{}) catch |err| {
                log.warn("resource upload failed: {}", .{err});
                self.clearBatchResources();
                return;
            };
            const next_prepared = pending.publish() catch |err| {
                log.warn("resource upload publish failed: {}", .{err});
                self.clearBatchResources();
                return;
            };
            if (self.prepared) |*old| old.retireNow();
            self.prepared = next_prepared;
            self.prepared_key = manifest_key;
            prepared = if (self.prepared) |*current| current else unreachable;
    ...
    fn ensureSparklinePicture(self: *Renderer) !*snail.PathPicture {
        if (self.sparkline_picture) |picture| return picture;
        ...
        picture.* = try builder.freeze(.{
            .persistent_allocator = self.allocator,
            .scratch_allocator = self.frame_arena.allocator(),
        });
        errdefer picture.deinit();
        self.sparkline_picture = picture;
        return picture;
    }

    fn addSparklineInstances(self: *Renderer, op: SparklineOp) !void {
        ...
        try buildSparklineInstances(frame_allocator, &instances, op);
        if (instances.items.len == 0) return;

        const picture = try self.ensureSparklinePicture();
    ...
    fn clearPrepared(self: *Renderer) void {
        if (self.prepared) |*prepared| {
            prepared.retireNow();
            self.prepared = null;
            self.prepared_key = null;
        }
    }
};

fn resourceManifestKey(resources: *const snail.ResourceSet) u64 {
    var hasher = std.hash.Wyhash.init(prepared_manifest_seed);
...
fn buildSparklineInstances(
    ...
    // Mirror reserves half_h for each half; single-series uses the full height.
    const series_h = if (op.mirror) op.h * 0.5 else op.h;
    const center_y = op.y + op.h * 0.5;
    const bottom_y = op.y + op.h;
    // Reserve room for the bar count we render. The first and last bars
    // are the cross-fading edge bars — their alpha is driven by `scroll`,
    // so at any given frame ~count-1 bars are fully bright.
    const content_w = @as(f32, @floatFromInt(count)) * bar_w +
        @as(f32, @floatFromInt(if (count > 0) count - 1 else 0)) * bar_gap;
    const skew_pad = @abs(op.skew) * series_h;
    const total_w = content_w + skew_pad * 2.0;
    const origin_x = op.x + @max(0.0, op.w - total_w) * 0.5 + skew_pad;

    for (0..count) |i| {
        // Physical scroll: shift each bar's center leftward by scroll*step.
        const center_x = origin_x + bar_w * 0.5 +
            (@as(f32, @floatFromInt(i)) - op.scroll) * step;

        // Cross-fade the leftmost (sliding off) and rightmost (entering) bars.
        var edge_alpha: f32 = 1.0;
        if (count > 1) {
            if (i == 0) edge_alpha = 1.0 - op.scroll;
            if (i == count - 1) edge_alpha = op.scroll;
        }
        ...
        if (op.mirror) {
            if (i < count2) {
                var tint = op.color2;
                tint[3] *= edge_alpha;
                try appendSparklineMirroredHalf(
                    allocator,
                    instances,
                    center_x,
                    center_y,
            ...
            var tint = if (i < op.colors.len) op.colors[i] else op.color;
            tint[3] *= edge_alpha;
            try appendSparklineBottomBar(
                allocator,
                instances,
                center_x,
                bottom_y,
...
fn appendSparklineBottomBar(
    ...
    if (value < 0 or full_h <= 0 or color[3] <= 0) return;
    const bar_h = @max(@max(min_bar_height, 0.0), value * full_h);
    if (bar_h <= 0) return;
...
fn appendSparklineMirroredHalf(
    allocator: std.mem.Allocator,
    instances: *std.ArrayListUnmanaged(snail.Override),
    center_x: f32,
    center_y: f32,
    ...
    if (solid_h > 0) {
        try appendSparklineSliceInstance(allocator, instances, center_x, center_y, bar_w, 0, solid_h, skew, upper, color);
    }
    ...
    for (0..sparkline_fade_slices) |i| {
        const f0 = @as(f32, @floatFromInt(i)) / @as(f32, @floatFromInt(sparkline_fade_slices));
        const f1 = @as(f32, @floatFromInt(i + 1)) / @as(f32, @floatFromInt(sparkline_fade_slices));
        ...
        var faded = color;
        faded[3] *= alpha;
        try appendSparklineSliceInstance(allocator, instances, center_x, center_y, bar_w, d0, d1, skew, upper, faded);
    }
}

fn appendSparklineSliceInstance(
    allocator: std.mem.Allocator,
    instances: *std.ArrayListUnmanaged(snail.Override),
    center_x: f32,
    center_y: f32,
